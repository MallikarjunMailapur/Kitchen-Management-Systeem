...
WHERE
    E.Shift = 'Night'
    AND O.Date = (SELECT MAX(Date) FROM Orders WHERE OrderID = P.OrderID);



--Nested query:  retrieve the number of days remaining for the expiry of items from the current date:
...
FOR EACH ROW
BEGIN
  INSERT INTO OldEmployees (EmployeeID, Name, Position, ContactDetails, Salary, HiredDate, ResignedDate, Shift)
  VALUES (OLD.EmployeeID, OLD.Name, OLD.Position, OLD.ContactDetails, OLD.Salary, OLD.HiredDate, CURDATE(), OLD.Shift);
END;
...
--function:
DELIMITER //
CREATE FUNCTION CalculateAnnualProfit(months INT)
RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN
    DECLARE total_salary_expenses DECIMAL(10, 2);
    DECLARE total_inventory_expenses DECIMAL(10, 2);
    DECLARE total_income DECIMAL(10, 2);
    DECLARE annual_profit DECIMAL(10, 2);

    -- Calculate total salary expenses
    SELECT SUM(Salary * months) INTO total_salary_expenses
    FROM Employees;

    -- Calculate total inventory expenses
    SELECT SUM(UnitPrice * Quantity) INTO total_inventory_expenses
    FROM Inventory;

    -- Calculate total income from Orders table
    SELECT SUM(TotalPrice) INTO total_income
    FROM Orders;

    -- Calculate annual profit
    SET annual_profit = total_income - (total_salary_expenses + total_inventory_expenses);
...
--function call
SELECT CalculateAnnualProfit(2) AS AnnualProfit;


CREATE USER 'manager'@'localhost' IDENTIFIED BY '9999';
GRANT ALL PRIVILEGES ON project.* TO 'manager'@'localhost';
